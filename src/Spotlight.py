...
def getTempDirectory():
    userdir = userDirectory()
    tempdir = os.path.abspath(os.path.join(userdir, "temp"))
    return tempdir
...
def extractFramesToTemp(file):
    tempdir = createTempDirectory()
    frame = os.path.abspath(os.path.join(tempdir, "frame%d.png"))
    cmd = 'ffmpeg -r 1 -i "' + file + '"  -r 1 "' + frame + '"'
    os.system(cmd)
...
def OnAboutSpotlight():
    aboutString = """
    Spotlight-8 (8-bit)  ver. 2023.12.12
...
def OnNewSinceLastRelease():
    newReleaseString = """
    --- New in Spotlight-8 ver 2023.12.12 ---
    - Added support for any codec in AVI files by
      extracting every frame to a temporary folder
      using ffmpeg
    ...
    - Fixed a problem with filenames not lining up properly 
      when they were certain lengths.
